...
const MultiStepForm = ({
  ...
  const handleSave = async (nextStep = null) => {
    saveHandler(nextStep, ["title"],"/api/announcement", `/api/announcement/${formData._id}/`, "Announcement saved Successfully!", "announcement", errors, formData, setErrors, handleChange, setReload, reload, closeForm);
  };
  const renderStep = () => {
    switch (step) {
      case 1:
        ...
        <FormCenteringContainer >
          <ModalContainer>
            <HeaderContainer>
              <Heading>
              {
                  isViewMode ? "View Announcement" : (isEditMode ? "Update Announcement" : "Create Announcement")
              }
              </Heading>
              <CloseButtonContainer>
            ...
            {
              isViewMode ? (
                <ViewAnnouncement announcement={formData}/>
              ) : (
                <>
                  {renderStep()}
                  <BottomButtonsContainer>
