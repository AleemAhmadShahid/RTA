...
const StepOne = ({ formData, errors, handleChange }) => {
  ...
  const [performanceEvaluationSkills, setJobDescriptionSkills] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await createGetRequest("/api/performanceEvaluation/criteria/");
        if (data.status === 404) {
          setSkills([]);
          return;
        }
    ...
    <FormStep active>
      ...
      <FormGroup>
        ...
        <FormInput
          ...
          placeholder={" Title"}
          onChange={(e) =>
            handleChange("period.startDate", e.target.value)
          }
          onBlur={(e) =>
            handleChange("period.startDate", e.target.value)
          }
      ...
      <FormGroup>
        ...
        <FormInput
          ...
          placeholder={" Title"}
          onChange={(e) =>
            handleChange("period.endDate", e.target.value)
          }
          onBlur={(e) =>
            handleChange("period.endDate", e.target.value)
          }
      ...
      <FormGroup>
          <FormLabel>Guideline Document</FormLabel>
          <PictureUploadButton>
              Upload Document
              <input type="file" onChange={async (e) => {
                ...
                if (response.status === 200)
                  handleChange("guidelineDocument", response.url || response.id, true)
      ...
      <Step3Container>
        <BoxStep3Container>
          {performanceEvaluationSkills.map((item, index) => (
              ...
              <FormGroup>
                <FormLabel>Skills</FormLabel>
                  <EntriesDropdown
                    width="100%"
                    value= {{
                      label: skills.find(item => item._id === formData?.skills?.[index]?.skill)?.endDate,
        ...
        <Step3AddButton
          onClick={() => addItem(setJobDescriptionSkills, performanceEvaluationSkills)}
        >
          Add Skill
        </Step3AddButton>
      </Step3Container>
