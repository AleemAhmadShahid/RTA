...
const MultiStepForm = ({
  ...
  const handleSave = async (nextStep = null) => {
    saveHandler(nextStep, ["name"],"/api/jobDescription", `/api/jobDescription/${formData._id}/`, "JobDescription saved Successfully!", "jobDescription", errors, formData, setErrors, handleChange, setReload, reload, closeForm);
  };
  const renderStep = () => {
    switch (step) {
      case 1:
        ...
        <FormCenteringContainer >
          <ModalContainer>
            <HeaderContainer>
              <Heading>
              {
                  isViewMode ? "View JobDescription" : (isEditMode ? "Update JobDescription" : "Create JobDescription")
              }
              </Heading>
              <CloseButtonContainer>
            ...
            {
              isViewMode ? (
                <ViewJobDescription jobDescription={formData}/>
              ) : (
                <>
                  {renderStep()}
                  <BottomButtonsContainer>
