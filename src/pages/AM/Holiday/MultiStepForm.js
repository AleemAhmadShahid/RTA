...
const MultiStepForm = ({
  ...
  const handleSave = async (nextStep = null) => {
    saveHandler(nextStep, ["name", "startTime", "endTime"],"/api/holiday", `/api/holiday/${formData._id}/`, "Holiday saved Successfully!", "holiday", errors, formData, setErrors, handleChange, setReload, reload, closeForm);
  };
  const renderStep = () => {
    switch (step) {
      case 1:
        ...
        <FormCenteringContainer >
          <ModalContainer>
            <HeaderContainer>
              <Heading>
              {
                  isViewMode ? "View Holiday" : (isEditMode ? "Update Holiday" : "Create Holiday")
              }
              </Heading>
              <CloseButtonContainer>
