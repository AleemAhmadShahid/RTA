...
const MultiStepForm = ({
  ...
  const handleSave = async (nextStep = null) => {
    ...
    saveHandler(nextStep, ["candidate"] ,"/api/offer", `/api/offer/${formData._id}/`, "Offer updated Successfully!", "offer", errors, copyFormData, setErrors, handleChange, setReload, reload, closeForm);
    ...
    showForm && (
      <ModalOverlay>
        <FormCenteringContainer>
          <ModalContainer>
            <HeaderContainer>
              <Heading>{isEditMode ? "Update Offer" : "Schedule Offer"}</Heading>
              <CloseButtonContainer>
